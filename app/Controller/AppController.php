...
class AppController extends Controller {
  ...
  /**
   * log - logs the message based on the log level and sends to FirePHP
   * 
   * @param type $msg
   * @param type $type 
   */
  public function log($msg, $comment = '', $type = 'LOG_APP_DEBUG') {
	# Write to firephp
	$debugLevels = Configure::read('App.debugLevels');
	$debugLevel = Configure::read('App.debugLevel');
	if (array_key_exists($type, $debugLevels)) {
	  $intLevel = $debugLevels[$type];
	} else {
	  $intLevel = 0;
	}
	if ($intLevel <= $debugLevel) {
	  $header = $this->toString() . '->' . $this->GetCallingMethod();
	  if (is_array($msg)) {
		if (class_exists('FireCake')) {
		  FireCake::log($header, 'LOG');
		  if ($comment <> '') {
			FireCake::log($comment, 'LOG');
		  }
		  FireCake::log($msg, 'LOG');
		}
		parent::log($header, $type);
		if ($comment <> '') {
		  parent::log($comment, $type);
		}
		parent::log($msg, $type);
	  } else {
		if (class_exists('FireCake')) {
		  ...
		  FireCake::log($header . ': ' . $msg, 'LOG');
		}
		if ($comment <> '') {
		  parent::log($comment, $type);
		}
		parent::log($header . ': ' . $msg, $type);
	  }
	} else {
	  FireCake::log('Not logging: ' . $msg . ', comment=' . $comment . ', type=' . $type . ',debug=' . Configure::read('debug'));
	}
  }
  /*
   * GetCallingMethod
   * 
   * Gets the calling method of the routine that called this one.
   */
