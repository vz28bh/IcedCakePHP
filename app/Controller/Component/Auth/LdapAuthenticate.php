...
class LdapAuthenticate extends BaseAuthenticate {
  ...
  public function authenticate(CakeRequest $request, CakeResponse $response) {
    //CakeLog::write('debug', "Ldap->authenticate: settings:" . print_r($this->settings, true));

    $userModel = $this->settings['userModel'];
    list($plugin, $model) = pluginSplit($userModel);

    $fields = $this->settings['fields'];
    //CakeLog::write('debug', "Ldap->authenticate: fields:" . print_r($fields, true));
    //CakeLog::write('debug', "Ldap->authenticate: this->form:" . $this->form);
    //CakeLog::write('debug', "Ldap->authenticate: request->data:" . print_r($request->data[$this->form], true));
    if (empty($request->data[$this->form])) {
      //CakeLog::write('debug', "Ldap->authenticate: request empty");
      return false;
    }
    if (
      ...
      //CakeLog::write('debug', "Ldap->authenticate: fields empty");
      return false;
    }
    $dn = $this->_getDn($this->model->primaryKey, $request->data[$this->form][$fields['username']]);
    //CakeLog::write('debug', "Ldap->authenticate: dn = " . $dn);
    $return = $this->_findLdapUser(
            $dn, $request->data[$this->form][$fields['password']]
    );
    //CakeLog::write('debug', 'Ldap->authenticate: return=' . print_r($return,true));
    return $return;
  }
  ...
  function _findLdapUser($dn, $password) {
    //CakeLog::write('debug', 'Ldap->_findLdapUser: dn=' . $dn . ', password=' . $password);

    $authResult = $this->model->auth(array('dn' => $dn, 'password' => $password));
    //CakeLog::write('debug', 'Ldap->_findLdapUser: authResult=' . $authResult);
    if ($authResult == 1) {
      $user = $this->model->find('first', array('scope' => 'base', 'targetDn' => $dn));
      //CakeLog::write('debug', 'Ldap->_findLdapUser: user=' . print_r($user, true));
  ...
  function _getDn($attr, $query) {
    //CakeLog::write('debug', 'Ldap->_getDn: $attr:' . $attr . ', $query:' . $query);
    $userObj = $this->model->find('first', array('conditions' => "$attr=$query", 'scope' => 'sub'));
    //CakeLog::write('debug', 'Ldap->_getDn: userObj:' . print_r($userObj, true));
  ...
  function existsOrCreateSQLUser($user) {
    ...
    if (isset($result) && !empty($result)) {
      ...
      if (isset($user['displayname']) && !empty($user['displayname']))
