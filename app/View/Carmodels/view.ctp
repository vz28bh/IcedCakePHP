...
<div class="row">
  <div class="related span10 offset2">
	...
	<div class="btn-toolbar">
	  <?php echo $this->JqueryDialog->link(__('New Picture'), array('controller' => 'carpictures', 'action' => 'add', $carmodel['Carmodel']['id']), array('class' => 'btn btn-mini', 'data-callback' => 'reload_window')); ?>			
	</div>
	
	<?php if (!empty($carmodel['Carpicture'])): ?>
  	<table class="table">
  	  <tr>
  		<th><?php echo __('Id'); ?></th>
  		<th><?php echo __('Name'); ?></th>
		...
		<?php
		$i = 0;
		foreach ($carmodel['Carpicture'] as $carpicture):
		  ?>
		  <tr>
			<td><?php echo $carpicture['id']; ?></td>
			<td><?php echo $carpicture['path']; ?></td>
  		<td>
			<?php
			$thumbnail = $this->Html->image($carpicture['thumbpath']);
			$image = Router::url($carpicture['path'], true);
			echo $this->Html->link($thumbnail, $image, array('escape' => false, 'class' => 'lightbox'));
			?>&nbsp;
  		</td>
		<td class="actions">
			  <div class="btn-group">
				<?php echo $this->Html->link('<i class="icon-eye-open"></i>', array('controller' => 'options', 'action' => 'view', $carpicture['id']), array('escape' => false, 'title' => __('View'), 'class' => 'btn btn-mini')); ?>
				<?php echo $this->JqueryDialog->link('<i class="icon-edit"></i>', array('controller' => 'options', 'action' => 'edit', $carpicture['id']), array('escape' => false, 'title' => __('Edit'), 'class' => 'btn btn-mini', 'data-callback' => 'reload_window')); ?>
				<?php echo $this->Html->link('<i class="icon-remove"></i>', array('controller' => 'options', 'action' => 'delete', $carpicture['id']), array('escape' => false, 'title' => __('Delete'), 'class' => 'btn btn-danger btn-mini'), __('Are you sure you want to delete # %s?', $carpicture['id'])); ?>
			  </div>
			</td>
		  </tr>
		<?php endforeach; ?>
