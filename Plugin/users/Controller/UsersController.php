...
class UsersController extends UsersAppController {
  ...
  public function login() {
    CakeLog::write('debug','UsersController->login: start');
    if ($this->request->is('post')) {
      if ($this->Auth->login()) {
        $this->User->id = $this->Auth->user('id');
        CakeLog::write('debug','UsersController->login: id='.$this->Auth->id.', user='.print_r($this->Auth->user,true));
  ...
  public function logout() {
    ...
    $this->Cookie->destroy();
    $this->Session->setFlash(sprintf(__d('users', '%s you have successfully logged out'), Inflector::camelize($user[$this->{$this->modelClass}->displayField])));
